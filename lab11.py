...
class Brand(Boat):
    ...
    def __str__(self):
        return (
            "How many horses? " + str(self.horsepower) + " and mpg is " + str(self.mpg)
        )
...
def test_Boat():
    ...
    """
    bay = Boat()
    assert bay.get_size() == 10
    assert bay.get_price() == 100
    assert str(bay) == "What is the price per foot for this boat? " + str(bay.ppf())
    bay.set_size(100)
    bay.set_price(500)
    assert bay.get_size() == 100
    assert bay.ppf() == 5
    buoy = Boat()
    assert bay != buoy
    assert buoy != bay
    assert bay.ppf != buoy.ppf
    assert eval(repr(bay)) != buoy
    print("Ahoy")
